...
MACHINES=("lab2-27.cs.mcgill.ca" "lab2-29.cs.mcgill.ca" "lab2-3.cs.mcgill.ca" "lab2-4.cs.mcgill.ca" "lab2-8.cs.mcgill.ca")
...
tmux new-session \; \
	...
	select-pane -t 4 \; \
	send-keys "ssh -t ${MACHINES[3]} \"cd $(pwd) > /dev/null; echo -n 'Connected to '; hostname; ./run_server.sh Customers ${MACHINES[0]} ${MACHINES[1]} ${MACHINES[2]}\"" C-m \; \
	select-pane -t 0 \; \
